...
export async function getToken (idCaptcha: string) {
  try {
    const token = await promiseRequestHttpGet(`${configJson.solution.url}?key=${process.env.SECRET_CAPTCHA_TOKEN}&action=get&id=${idCaptcha}&json=true`) as IRequestCaptcha
    console.log(token)
...
export async function getId (siteKey: string, siteUrl: string) {
  const idCaptcha = await promiseRequestHttpGet(`${configJson.submit.url}?key=${process.env.SECRET_CAPTCHA_TOKEN}&json=true&method=userrecaptcha&googlekey=${siteKey}&pageurl=${siteUrl}`) as IRequestCaptcha
